...
export default function Favoritos({viewSaved, setViewSaved, salvar, salvas, frases}){
    ...
    return(
        <>
            <SaveIcon title={`Ver salvas`} onClick={()=> setViewSaved(prevViewSaved => !prevViewSaved)} className={`[&_*]:duration-100 [&_path]:stroke-gray-500 [&_path]:hover:fill-gray-500 [&_path]:fill-gray-500 left-4 fixed top-5`} />

            {viewSaved && <ul className='fixed left-0 md:w-[450px] w-full bg-gray-300 dark:bg-gray-700/50 shadow-2xl h-full z-[999998] overflow-y-auto flex flex-col gap-8 salvos backdrop-blur-sm top-0 [box-shadow:inset_0_0_10px_#fff1]'>
                    <button title="ver salvos" 
